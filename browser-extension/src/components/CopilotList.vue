  ...
  <div class="min-h-screen flex flex-col">
    <main class="flex-grow">
      <div class="header items-center py-3 px-1.5 copilot-list-header">
        <div class="text-[20px] font-semibold">My copilots</div>
        <div class="actions">
      ...
      <div class="copilot-list">
        <div class="list-row">
          <div class="flex items-center py-4 text-base cursor-pointer" @click="showAddCopilot = true"><circle-plus-icon /> Add copilot</div>
        </div>
        <div v-for="(agent, index) in agents" :key="agent.manifest.id" class="list-row">
          <div class="copilot-item list-item items-center py-3" @click="$emit('activateAgent', agent.manifest.id)">
            ...
            <div class="copilot-name">{{ agent.manifest.name }}</div>
            &nbsp;<span class="font-light">Copilot</span>
          </div>
    ...
    <Footer class="py-10" />
    <ModalForm title="Remove copilot" :show="deletingIndex >= 0" @close="closeDeletionConfirmation" @save="confirmRemoval" button-text="Remove">
      Are you sure you want to remove the copilot {{ agents ? agents[deletingIndex].manifest.name : "" }}?
    </ModalForm>
    <CopilotConfig
      :show="configAgent !== undefined"
      @close="configAgent = undefined"
      :agent-id="configAgent?.manifest.id || ''"
      :agent-name="configAgent?.manifest.name || ''"
